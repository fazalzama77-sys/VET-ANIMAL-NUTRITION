/*
 * Safe renderer for ICAR-IVRI Animal Nutrition department announcements.
...
(() => {
    ...
    const SEEN_KEY = 'vanut-event-announcements-seen';
    ...
    const normalizeConfig = () => {
        const source = window.IVRI_NUTRITION_EVENTS_CONFIG;
        ...
        return {
            ...
            title: safeText(source.sectionTitle, 'Animal Nutrition Announcements'),
    ...
    const buildCard = (event) => {
        const card = make('article', 'vanut-event-card');
        if (event.featured === true) card.classList.add('is-featured');

        const meta = make('div', 'vanut-event-meta');
        meta.append(make('span', 'vanut-event-category', safeText(event.category, 'Academic Event')));
        if (event.published !== true) meta.append(make('span', 'vanut-event-draft', 'DRAFT PREVIEW'));

        const title = make('h3', '', safeText(event.title));
        const date = make('p', 'vanut-event-date');
        date.textContent = `📅 ${formatDate(event.date)}`;

        card.append(meta, title, date);
        const speaker = safeText(event.speaker);
        if (speaker) card.append(make('p', 'vanut-event-speaker', speaker));
        const description = safeText(event.description);
        if (description) card.append(make('p', 'vanut-event-description', description));

        const actions = make('div', 'vanut-event-actions');
        const watch = makeLink(event.youtubeUrl, 'Watch Video', 'vanut-event-action vanut-event-youtube', 'fas fa-play');
        const register = makeLink(event.registrationUrl, 'View Details', 'vanut-event-action vanut-event-register', 'fas fa-arrow-up-right-from-square');
    ...
    const injectStructuredData = (events) => {
        ...
        const payload = published.map((event) => ({
            ...
            organizer: { '@type': 'Organization', name: 'ICAR-Indian Veterinary Research Institute', url: 'https://animalnutrition.ivri.res.in/' }
        }));
        const script = document.createElement('script');
        script.type = 'application/ld+json';
        script.dataset.vanutEvents = 'true';
    ...
    const showAnnouncement = (event) => {
        ...
        if (document.querySelector('.vanut-event-dialog')) return;

        const overlay = make('div', 'vanut-event-dialog');
        const dialog = make('section', 'vanut-event-dialog-card');
        dialog.setAttribute('role', 'dialog');
        dialog.setAttribute('aria-modal', 'true');
        dialog.setAttribute('aria-labelledby', 'vanut-event-dialog-title');

        const close = make('button', 'vanut-event-dialog-close');
        close.type = 'button';
        close.setAttribute('aria-label', 'Close announcement');
        close.innerHTML = '&times;';

        dialog.append(close, make('div', 'vanut-event-dialog-kicker', safeText(event.category, 'Upcoming Animal Nutrition Event')));
        const title = make('h2', '', safeText(event.title));
        title.id = 'vanut-event-dialog-title';
        dialog.append(title, make('p', 'vanut-event-dialog-date', formatDate(event.date)));
        if (safeText(event.description)) dialog.append(make('p', 'vanut-event-dialog-description', safeText(event.description)));

        const actions = make('div', 'vanut-event-dialog-actions');
        const watch = makeLink(event.youtubeUrl, 'Open Video', 'vanut-event-action vanut-event-youtube', 'fas fa-play');
        const register = makeLink(event.registrationUrl, 'View details', 'vanut-event-action vanut-event-register', 'fas fa-arrow-up-right-from-square');
        if (watch) actions.append(watch);
        if (register) actions.append(register);

        const later = make('button', 'vanut-event-later', 'Not now');
    ...
    const mountEventsSection = () => {
        try {
            ...
            if (!anchor) return;
            if (document.getElementById('vanut-events')) return;

            const section = make('section', 'vanut-events-section');
            section.id = 'vanut-events';
            section.setAttribute('aria-label', config.title);

            const header = make('div', 'vanut-events-header');
            const headingWrap = make('div');
            const eyebrow = make('div', 'vanut-events-eyebrow', 'ACADEMIC ANNOUNCEMENTS');
            const title = make('h2', '', config.title);
            headingWrap.append(eyebrow, title);
            if (config.subtitle) headingWrap.append(make('p', 'vanut-events-sub', config.subtitle));
            header.append(headingWrap);

            const channel = makeLink(config.youtubeUrl, config.youtubeLabel, 'vanut-events-channel', 'fas fa-video');
            if (channel) header.append(channel);
            section.append(header);

            if (config.events.length) {
                const grid = make('div', 'vanut-events-grid');
                config.events.forEach((event) => grid.append(buildCard(event)));
                section.append(grid);
            } else {
                const empty = make('div', 'vanut-events-empty');
                const emptyText = make('div');
                emptyText.append(
                    make('strong', '', config.emptyMessage),
                    make('span', '', 'New seminars, webinars, and feed technology workshops will be posted here.')
                );
            ...
            console.warn('Animal Nutrition events section skipped:', error);
        }
    };
